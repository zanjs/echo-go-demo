...
func Start() {
	...
	dataURL := conf.Config.Database.Db_user + ":" + conf.Config.Database.Db_pass + "@(" + conf.Config.Database.Db_host + ":" + strconv.Itoa(conf.Config.Database.Db_port) + ")/" + conf.Config.Database.Db_name
	logger.Info(dataURL)
	sqlSession, err := sqlx.Open("mysql", dataURL+"?parseTime=true")
	if err != nil {
